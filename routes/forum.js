...
function loginRequired(req, res, next) {
  if (!req.isAuthenticated()) {
    req.flash('info', 'Vous devez vous connecter pour continuer.')
...
function adminRequired(req, res, next) {
  if (req.isAuthenticated()) {
    if (!req.user.admin) {
      req.flash('error', 'Seuls les admins ont accès à cette page.')
      return res.redirect('back')
    }
    next()
  }else {
...
router
  ...
  //Result page
  .get('/resultat', (req, res)=>{
    res.render('resultat')
  })  
  
  //ResultPhoto page
  .get('/resultP', (req, res)=>{
    res.render('resutphoto')
  })
